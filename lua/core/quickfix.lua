...
local function quickfix_replace()
  ...
  local cmd = string.format("cdo %%s/%s/%s/g | update", vim.fn.escape(search, '/\\'), vim.fn.escape(replacement, '/\\'))
  ...
  if success then
    vim.notify(string.format("Successfully replaced '%s' with '%s' in %d files", 
      search, replacement, #qf_list), vim.log.levels.INFO)
  else
